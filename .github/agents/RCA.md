...
1. You MUST ONLY search, read, evaluate, and extract SQL from the `my_queries` directory.
2. You MUST COMPLETELY IGNORE all files, code snippets, and context located outside of the `my_queries` directory.
3. You are FORBIDDEN from using tools to list or read files in any other directory. 
4. If a relevant SQL query cannot be found inside `my_queries`, do NOT fall back to other folders. Instead, state: "No relevant queries found in the designated directory.
